...
class FredILP:
    def __init__(self, cints: fsac.canonInts):
        row_to_ridxs: defaultdict[tuple[np.uint8, ...], list] = defaultdict(list)
        for idx, row in enumerate(cints.get_matrix()):
            row_to_ridxs[tuple(row)].append(idx)
        self.rows, self.ridxs = zip(*row_to_ridxs.items())
        self.lengths = tuple(interval.end - interval.start for interval in cints.intervals)
...
def informative_segs(tint, remaining_rids):
    ...
    for j in range(1, M - 1):
        if len(seg_content[j]) == 1 and (
            seg_content[j - 1] == seg_content[j] == seg_content[j + 1]
        ):
            informative[j] = False
    return informative
