...
def read_split(split_tsv):
    ...
    for line in open(split_tsv):
        if line[0] == "#":
            ...
            assert (
                not tint["id"] in tints
            ), "Transcriptional interval with id {} is repeated!".format(tint["id"])
            assert all(
                a[1] < b[0]
                for a, b in zip(tint["intervals"][:-1], tint["intervals"][1:])
            ), tint["intervals"]
...
def smooth_threshold(threshold):
    ...
    while True:
        x = len(smooth)
        y = threshold / (1 + ((threshold - 0.5) / 0.5) * exp(-0.05 * x))
...
def get_unaligned_gaps_and_polyA(read, segs):
    ...
    for char in ["A", "T"]:
        ...
        for i, l, p in find_longest_poly(
            read["seq"], s=s, e=e, step=step, char=sc_char
        ):
            if l < 20 or p < 0.85:
                continue
    ...
    if len(s_polys) > 0:
        ...
        read["gaps"].add("S{}_{}:{}".format(char, l, poly_to_gene_gap_size))
        read["gaps"].add("SSC:{}".format(i))
    else:
        read["gaps"].add("SSC:{}".format(q_ssc_pos))
    e_polys = list()
    for char in ["A", "T"]:
        s = q_esc_pos
        e = read["length"]
...
def run_segment(segment_args):
    ...
    LOG_FILE = open(
        "{}/{}/segment_{}_{}.log".format(outdir, contig, contig, tint_id), "w+"
    )
    tints = read_split(
    ...
    read_sequence(
        tint=tint,
        reads_tsv="{}/{}/reads_{}_{}.tsv".format(split_dir, contig, contig, tint_id),
    )
    out_file = open(
        "{}/{}/segment_{}_{}.tsv".format(outdir, contig, contig, tint_id), "w+"
    )
    segment(
        tint,
        sigma,
...
def segment(
    ...
    tint["segs"] = [
        (s, e)
        for s, e in zip(tint["final_positions"][:-1], tint["final_positions"][1:])
    ]
    for read in tint["reads"]:
        read["data"].pop()
...
def main():
    ...
    for idx, (contig, tint_id) in enumerate(mapper(run_segment, segment_args)):
        if not idx % ceil(len(segment_args) / 100) == 0:
            continue
        print(
            "[freddie_segment] Done with {}/{} tints ({:.1%})".format(
                idx, len(segment_args), idx / len(segment_args)
            ),
        )
    if args.threads > 1:
        p.close()
